...
function ProgressRing({ streak, goal = 7 }: { streak: number; goal?: number }) {
  ...
  return (
    <svg viewBox="0 0 80 80" className="h-20 w-20 shrink-0" role="img" aria-label={`连续练习 ${streak} 天`}>
      <circle cx="40" cy="40" r={r} fill="none" stroke="rgba(56,52,78,0.12)" strokeWidth="7" />
      <circle
        cx="40"
        cy="40"
        r={r}
      ...
      />
      <text x="40" y="38" textAnchor="middle" fontSize="18" fontWeight={600} fill="#26215C">
        {streak}
      </text>
      <text x="40" y="54" textAnchor="middle" fontSize="9" fill="#5F5E5A">
        连续天
      </text>
    </svg>
  );
          ...
          <Link
            ...
            className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm outline-none transition focus-visible:ring-2 focus-visible:ring-brand/60 ${
              active
                ...
                : "text-brand-sec/80 hover:bg-white/50"
            }`}
          >
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      {/* 桌面端：左侧固定玻璃侧栏 */}
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-white/50 bg-white/40 px-5 py-6 backdrop-blur-xl lg:flex">
